...
<template>
    <div class="min-h-screen flex items-center justify-center bg-gray-50 p-4 sm:p-6">
        <section class="w-full max-w-md mx-auto bg-white rounded-lg shadow-xl p-6 sm:p-8 md:p-10">
            ...
            <Form v-slot="$form" :value="items" :resolver="resolver" method="POST" @submit="onSubmit" class="space-y-5">
                ...
                <IftaLabel class="block">
                    <InputText
                        ...
                        :class="['w-full', { 'p-invalid': $form.username?.invalid }]"
                        aria-describedby="username-error"
                    />
                    <label for="username">Username:</label>
                    <Message v-if="$form.username?.invalid" severity="error" :pt="{ root: { class: 'mt-2' }, text: { class: 'text-sm' } }">{{ $form.username.error.message }}</Message>
                </IftaLabel>
                ...
                <IftaLabel class="block">
                    ...
                    <Message v-if="$form.verifyPassword?.invalid" severity="error" :pt="{ root: { class: 'mt-2' }, text: { class: 'text-sm' } }">{{ $form.verifyPassword.error.message }}</Message>
                </IftaLabel>
                ...
                <IftaLabel class="block">
                    <InputText
                        ...
                        name="phone"
                        v-model="items.phone"
                        :class="['w-full', { 'p-invalid': $form.phone?.invalid }]"
                        aria-describedby="phone-error"
                    />
                    <label for="phone">Phone:</label>
                    <Message v-if="$form.phone?.invalid" severity="error" :pt="{ root: { class: 'mt-2' }, text: { class: 'text-sm' } }">{{ $form.phone.error.message }}</Message>
                </IftaLabel>

                <div class="flex items-center space-x-2">
                    <Checkbox v-model="items.privacyFlag" :binary="true" :input-id="'privacyFlag'" :invalid="$form.privacyFlag?.invalid" />
                    <label for="privacyFlag" class="text-gray-700">I accept with this</label>
                    <span @click="dialogPrivacyPolicy = true" class="text-blue-600 hover:text-blue-800 cursor-pointer text-sm font-medium">Privacy Terms</span>
                    <Message v-if="$form.privacyFlag?.invalid" severity="error" :pt="{ root: { class: 'ml-0 mt-2' }, text: { class: 'text-sm' } }">{{ $form.privacyFlag.error.message }}</Message>
                </div>

                <Dialog v-model:visible="dialogPrivacyPolicy" modal header="Privacy Policy for Room Visualizer"
                    :style="{ width: '90vw', maxWidth: '800px' }"
                    :breakpoints="{ '960px': '85vw', '640px': '95vw' }"
                    :pt="{ content: 'overflow-y-auto max-h-[80vh] text-sm' }"> <p class="text-base sm:text-lg font-semibold pb-2">Effective Date: 1 May 2025</p>
                    <p class="text-base font-semibold pb-1">1. Introduction</p>
                    ...
                    <p class="text-base font-semibold pb-1">2. Information We Collect</p>
                    <p class="text-sm pb-2 pl-5">
                    ...
                    <p class="text-base font-semibold pb-1">3. Use of Information</p>
                    <p class="text-sm pb-2 pl-5">
                    ...
                    <p class="text-base font-semibold pb-1">4. Sharing of Information</p>
                    <p class="text-sm pb-2 pl-5">
                    ...
                    <p class="text-base font-semibold pb-1">5. Data Security</p>
                    <p class="text-sm pb-2 pl-5">
                    ...
                    <p class="text-base font-semibold pb-1">6. Your Rights and Choices</p>
                    <p class="text-sm pb-2 pl-5">
                    ...
                    <p class="text-base font-semibold pb-1">7. International Transfers</p>
                    <p class="text-sm pb-2 pl-5">
                    ...
                    <p class="text-base font-semibold pb-1">8. Changes to This Privacy Policy</p>
                    <p class="text-sm pb-2 pl-5">
                ...
                <Dialog v-model:visible="dialogSuccess" modal header="Success"
                    ...
                    <p class="m-0 text-lg text-center font-semibold text-green-700">
                        Congratulations🎉🎉🎉 You have created an account successfully. Please verify your email and get started!
                    </p>
                </Dialog>

                <div class="text-center text-gray-700 mt-6">
                    Already have an account? <RouterLink to="/login" class="text-blue-600 hover:text-blue-800 font-medium">Log in</RouterLink>
                </div>
            </Form>
        </section>
    </div>
